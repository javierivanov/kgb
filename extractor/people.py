...
for product in product_list:
    persons = df[df[product] > s[s.index == product].values[0]][['Team Member', 'Shift']].values
    persons_sme = df[df[product] > sme[sme.index == product].values[0]][['Team Member', 'Shift']].values
    if product in mappings:
        product = mappings[product]
    for person, shift in persons:
        query  = ' merge (p: Person {name: "%s", shift: "%s"})' % (person, shift)
        query += ' merge (c: Component {name: "%s"})' % product
        query += ' merge (p)-[:KNOWS]->(c)'
        print(query)
        graph.run(query)
    
    for person, _ in persons_sme:
